...
app.layout = html.Div(id='container', style=dict(fontFamily='Garamond'), children=[
    dbc.Container([
        ...
        dbc.Col(id='headline2', width=12, style=dict(textAlign='center'), children=html.H3(f"{org_name}", id='org-name-label')),
        html.Br(),
        dbc.Col(id='data-table', width=dict(size=6, offset=3),
                    ...
                    dbc.Col(width=dict(size=4, offset=3), #style=dict(backgroundColor='crimson'),
                            children=
                                daq.Gauge(
                                    ...
                                    value=predicted_probability * 10,
        ...
        html.Div(id='risk',
                 children=[
                        html.H3(risk_assess, id='risk-label', style=dict(textAlign='center')),
                        dcc.Interval(
...
@app.callback(
    Output('gauge', 'value'),
    Output('org-name-label', 'children'),
    Output('table', 'data'),
...
def move_needle(clicks):
    if clicks is None:
        ...
        # create risk assessment from risk table data
        risk_table_cols = ['cost', 'availability', 'quantity_total']
        data = ticket_table.loc[:, risk_table_cols].to_dict('records')
        risk_assess = risk(r)
        print(predicted_probability)
        return predicted_probability * 10, org_name, data, risk_assess
